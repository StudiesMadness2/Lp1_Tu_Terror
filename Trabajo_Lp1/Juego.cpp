...
void Juego::intentamosInteractuarAvatar(int numAA, int numAd, int numP, Arma *ArmA, Armadura *Armd, PocionCuracion *Poc) {
    char tipo;
                        ...
                        case ARTEFACTO:
                            ...
                            switch (tipo){
                                case 0:
                                    //TIPO ARMA
                                    val = rand() % numAA;
                                    this->avatar.SetSaco(ArmA[val]);
                                    break;
                                case 1:
                                    //TIPO ARMADURA
                                    val = rand() % numAd;
                                    this->avatar.SetSaco(Armd[val]);
                                    break;
                                case 2:
                                    //TIPO POCION
                                    val = rand() % numP;
                                    this->avatar.SetSaco(Poc[val]);
                                    break;
                            }
                            if (this->avatar.GetSacoIndice()<10) LaberintoActual.getCelda()[y + f][x + k].SetTipo(ADENTRO);
                            break;
                    }
